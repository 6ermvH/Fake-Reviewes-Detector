...
def build_vectorizer(v_cfg: dict):
    print(
        f"Building vectorizer of type {v_cfg['type']} with max_features=\
                {v_cfg['max_features']} and ngram_range={v_cfg['ngram_range']}"
    )
    if v_cfg["type"] == "tfidf":
...
def build_model(m_cfg: dict):
    print(
        f"Building model of type {m_cfg['type']} with hyperparameters \
                {m_cfg['hyperparameters']}"
    )
    t = m_cfg["type"]
